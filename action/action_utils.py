...
def get_executor(use_gpu=True):
    torch_module = MobileNetV2(n_class=2,input_size=32)
    ...
    sd = torch.load('/home/ubilab/Source/Taxi_demand_detection_Xavier/action/check_points/ckpt.best.pth.tar')
    sd = sd['state_dict']
    model_dict = torch_module.state_dict()
    if isinstance(torch_module, MobileNetV2):
    ...
    torch_inputs = (torch.rand(1, 3, 32, 32),
                    torch.zeros([1, 3, 8, 8]),
                    torch.zeros([1, 4, 4, 4]),
                    torch.zeros([1, 4, 4, 4]),
                    torch.zeros([1, 8, 2, 2]),
...
def transform(frame: np.ndarray):
    # 480, 640, 3, 0 ~ 255
    frame = cv2.resize(frame, (32, 32))  # (224, 224, 3) 0 ~ 255
...
def main():
    ...
    buffer = (
        tvm.nd.empty((1, 3, 8, 8), ctx=ctx),
        tvm.nd.empty((1, 4, 4, 4), ctx=ctx),
        tvm.nd.empty((1, 4, 4, 4), ctx=ctx),
        tvm.nd.empty((1, 8, 2, 2), ctx=ctx),
        tvm.nd.empty((1, 8, 2, 2), ctx=ctx),
        tvm.nd.empty((1, 8, 2, 2), ctx=ctx),
        tvm.nd.empty((1, 12, 2, 2), ctx=ctx),
        tvm.nd.empty((1, 12, 2, 2), ctx=ctx),
